<script lang="ts">
  ...
  let {
    ...
    selectedFiles: File[];
    pageRange: string;
    rotateDegrees: (typeof rotationOptions)[number]["value"];
    isProcessing?: boolean;
    onPageRangeChange: (value: string) => void;
    onRotateDegreesChange: (value: (typeof rotationOptions)[number]["value"]) => void;
...
<div class="mb-4">
  <div class="mb-2 flex items-center gap-2">
    <label for="rotate-page-range" class="block text-sm text-muted-foreground">Pages</label>
    <Tooltip.Provider delayDuration={0} skipDelayDuration={0} disableHoverableContent>
      <Tooltip.Root>
        ...
        <Tooltip.Content side="top" class="max-w-72 leading-relaxed">
          1-3,5 means pages 1, 2, 3 and 5 will be rotated.
        </Tooltip.Content>
      </Tooltip.Root>
    </Tooltip.Provider>
  </div>
  <input
    id="rotate-page-range"
    value={pageRange}
...
<div class="mb-4">
  <label for="rotation" class="block text-sm text-muted-foreground mb-2">Rotation</label>
  <select
    id="rotation"
    value={rotateDegrees}
    onchange={(event) =>
      onRotateDegreesChange(
        Number((event.currentTarget as HTMLSelectElement).value) as (typeof rotationOptions)[number]["value"]
      )}
