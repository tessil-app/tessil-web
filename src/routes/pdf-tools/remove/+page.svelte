<script lang="ts">
  ...
  import RemoveTool from "$lib/features/pdf-tools/remove/RemoveTool.svelte";
  import {
    applyActionMode,
    createOutputItems,
    formatSize,
  ...
  const PAGE_URL = `${SITE_URL}/pdf-tools/remove`;
  const PAGE_TITLE = "Remove PDF Pages - JTransfer";
  const PAGE_DESCRIPTION =
    "Remove selected pages from a PDF locally in your browser with JTransfer.";
  ...
  function handleFiles(fileList: FileList) {
    ...
    warning =
      incoming.length > 1
        ? "Remove pages works with one PDF at a time. Using the first selected file."
        : null;
  }
  ...
  async function processRemove() {
    ...
    try {
      const processedFiles = await removePdfPages(selectedFiles[0], pageRange);
      outputs = createOutputItems(processedFiles);
      const result = await applyActionMode(processedFiles, actionMode);
      warning = result.warning;
    } catch (err) {
      error = err instanceof Error ? err.message : "Remove pages failed.";
    } finally {
      isProcessing = false;
    }
  }
  ...
  <div class="max-w-5xl mx-auto px-4 py-10">
    <div class="mb-8">
      <a
        ...
        >← All PDF tools</a
      >
      <h1 class="mt-3 text-4xl font-bold tracking-tight">Remove PDF Pages</h1>
      <p class="mt-2 text-muted-foreground max-w-2xl">
        Remove exact page ranges while keeping the rest untouched.
      </p>
    </div>
    ...
    <div class="grid gap-6 lg:grid-cols-[1.35fr_0.95fr]">
      ...
      <Frame.Root>
        <Frame.Panel>
          <h2 class="text-base font-semibold text-foreground">Remove Notes</h2>
          <ul class="mt-3 space-y-2 text-sm text-muted-foreground leading-relaxed">
            <li>Range format is <code>1-3,5</code>.</li>
            <li>The selected pages are removed from the output file.</li>
            <li>You must keep at least one page in the document.</li>
          </ul>
        </Frame.Panel>
      </Frame.Root>
    </div>
